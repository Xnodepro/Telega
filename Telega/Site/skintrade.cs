...
namespace Telega.Site
{
    class skintrade
    {
        ...
        public void INI(int index)
        {
            ...
            driver.Navigate().GoToUrl("http://skin.trade/authenticate");
        ...
        private void Get(HttpClientHandler handler, int id)
        {
         ...
         "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36");
            ...
            while (true)
            {
                try
                {
                    string dataRequest = HttpUtility.UrlEncode(GetDataTimForRequest());
                    var response = client.GetAsync("http://skin.trade/load_all_bots_inventory?hash=" + dataRequest).Result;
        ...
        private void ClickItem(dynamic itm)
        {
                ...
                {
                    foreach (var childItems in item.First)
                    {
                        try
                        {
                            foreach (var it in DataStruct.ItemsSearch)
                            {
                            if (Site == it.Site && childItems.m.Value.ToString().Replace(" ", "") == (it.Name).Replace(" ", ""))
                            {
                               // Program.Mess.Enqueue("[skintrade]" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "| Нашел предмет:" + it.Name);
                                ItemsInfo II = new ItemsInfo()
                                {
                                    Name = it.Name,
                                    Site = it.Site,
                                    Price = childItems.p.Value.ToString(),
                                    Floaat = childItems.f.Value.ToString(),
                                ...
                                {
                                 //   Program.Mess.Enqueue("[skintrade]" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "| Отправил предмет:" + it.Name);
        ...
        string GetDataTimForRequest()
        {
            DateTime dt = DateTime.Now;
            string[] temp = dt.ToString("R").Replace(",", "").Split(' ');
            var tem = temp[1];
            temp[1] = temp[2];
            temp[2] = tem;
            string full = "";
